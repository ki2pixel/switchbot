...
- **Reading**: Utilisez l'outil `fast_read_file` avec le paramètre `path="/absolute/path/to/file"`.
...
- **Updating**: Utilisez l'outil `edit_file` avec `path="/absolute/path/to/file"` pour synchroniser l'état du projet en fin de tâche.
- **Writing**: Utilisez l'outil `fast_write_file` avec `path="/absolute/path/to/file"` pour l'archivage ou l'initialisation (voir restrictions dans memorybankprotocol.md).
- **Listing projects**: Utilisez l'outil `fast_get_directory_tree` avec `path="/absolute/path/to/memory-bank"` pour diagnostiquer la connexion.
- **Listing files**: Utilisez l'outil `fast_list_directory` avec `path="/absolute/path/to/memory-bank"` pour lister les fichiers disponibles.
...
- **Task Management**: `shrimp-task-manager` for project planning and backlog management
- **Sequential Thinking**: `sequentialthinking_tools` for logical decomposition and validation
- **Fast Filesystem**: `edit_file`, `fast_read_multiple_files` for token-efficient operations
- **JSON Query**: `json_query_query_json`, `json_query_search_keys`, `json_query_search_values` for precise JSON manipulation
...
- **Token Efficiency**: Pour les fichiers >1000 lignes, utilisation obligatoire de `edit_file` plutôt que `edit_file` standard.
