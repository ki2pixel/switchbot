...
- **Routes & Formulaires (`routes.py`, `settings.html`)** : Renforcement des validations serveur des intervalles de sondage (`poll_interval_seconds` minimum relevé de 15s à 60s, et `idle_poll_interval_seconds` minimum relevé de 15s à 300s) et alignement des contraintes d'input HTML correspondantes.
...
- **Validation** : Exécution de la suite pytest complète (180 tests passés à 100%).
