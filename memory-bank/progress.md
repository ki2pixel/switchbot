## Terminé
[2026-07-10 15:26:00] - Suppression intégrale du support d'IFTTT de l'application
- **Suppression** : Retrait des fichiers `switchbot_dashboard/ifttt.py`, `tests/test_ifttt.py`, et `docs/guides/ifttt-setup.md`.
- **Backend & Moteur d'automatisation (`__init__.py`, `automation.py`, `aircon.py`, `routes.py`)** : Remplacement de la cascade d'automatisation à 3 niveaux par une cascade simplifiée à 2 niveaux (Scènes -> Direct API). Retrait de toute référence à `ifttt_client`, suppression des signatures de méthodes décisionnelles et des validateurs de formulaire.
- **Frontend (`settings.html`)** : Retrait de toute trace d'IFTTT de l'UI (le formulaire de configuration des webhooks IFTTT et le panneau résumé ont été retirés) avec ajustement ergonomique de la grille Bootstrap.
- **Tests & Docs** : Correction et adaptation de la suite de tests (`test_dashboard_routes.py`, `test_automation_service.py`, `test_adaptive_cooldown.py`, `test_backend_hardening.py`, `test_phase3.py`) pour éliminer les mocks d'IFTTT et valider le nouveau flux. Mise à jour de tous les documents d'architecture, de dépannage, de stratégie de test et des rapports de sécurité.
- **Validation** : Tous les 157 tests passent avec succès, maintenant la couverture de code globale à 77%.
