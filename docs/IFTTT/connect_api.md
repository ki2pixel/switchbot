...
Throughout these docs, we’ll use `{{service_id}}` as a placeholder for your service’s unique identifier, which can be found in the [Service tab](https://ifttt.com/mkt/general) of the IFTTT Platform under the **IFTTT service ID** heading.
...
| `500`  | There was an internal error. If this persists, [contact us](mailto:platform-support@ifttt.com). |
...
```http
GET /v2/connections/invalid_connection_id Host: connect.ifttt.com
```
...
2.  **Service-authenticated**: A request that includes an `IFTTT-Service-Key` header containing your service key, found in the [API tab](https://ifttt.com/mkt/api) of the IFTTT Platform under the **Service Key** heading. You can use this approach when you’re making calls from your backend servers to the API.
    

##### Example: Unauthenticated

```http
GET /v2/me Host: connect.ifttt.com
```

##### Example: Service-authenticated

```http
GET /v2/me Host: connect.ifttt.com IFTTT-Service-Key: 6e7c8978c07a3b5918a237b9b5b1bb70
```

##### Example: Service-authenticated with user_id

```http
GET /v2/me?user_id=123 Host: connect.ifttt.com IFTTT-Service-Key: 6e7c8978c07a3b5918a237b9b5b1bb70
```

##### Accessing user specific resources

Unless otherwise stated, most Connection API requests access user specific resources, so the `user_id` parameter is required. This `user_id` has to match the `id` that your service returns to IFTTT from its [User information endpoint](https://ifttt.com/docs/api_reference#user-information) for a given user. Some endpoints can return extra user-specific information if you send a valid `user_id`, while other endpoints require you to send a `user_id` when you call them with a service key.
...
![connect-url-screenshot](https://web-assets.ifttt.com/builds/assets/connect_url_screenshot-YrIPNtlb.png)
...
```http
POST /v2/provisional_access_code Host: connect.ifttt.com IFTTT-Service-Key: {{ifttt_service_key}} {
  "oauth_code": "foo"
}
```
...
1.  You're waiting for a trigger event webhook to come in and then use `["event_data"]["ingredients"]` to run an action. Learn more [here](#trigger-event-webhook).
2.  You're waiting for a connection enabled webhook to come in and then use that to send the user some helpful information. Learn more [here](#enabled-webhook).
...
- **type** (string): The type of object the event is for.
...
|-------|------|-------------|
| `sent_at` | number | The time in milliseconds when the webhook was sent. |
| `type` | string | The type of object the event is for. |
| `event` | string | The event that occurred. |
| `data` | object | Information about the object. |
| `data.connection_id` | string | The id of the connection. |
| `data.user_id` | string | The id of the connection user. |
| `data.user_timezone` | string | The user timezone. Default is "Pacific Time (US & Canada)". |
| `event_data` | object | Information about the event. |
...
This endpoint returns information about the authentication of the current request. You can use this to verify that you’re using a valid service key; you can also use it to determine whether the [`user_id` parameter](#accessing-user-specific-resources) you sent is currently connected to an IFTTT account and, if so, what that IFTTT account’s login is.
...
```http
GET /v2/me?user_id=123 Host: connect.ifttt.com
...
This endpoint can be used to provide details about a specific connection. If the request is [authenticated](#authentication) the response will include details specific to the authenticated user. If the authenticated user has enabled this connection, a `user_connection` object will be present, including the current configuration for [user trigger fields](#user-trigger-fields), [user query fields](#user-query-fields), and [user action fields](#user-action-fields) in a connection.
...
```http
GET /v2/connections/C8p3q9T6?user_id=123 HTTP/1.1
...
| `user_connection.user_features.user_feature_triggers.id` | (string) An ID referring to the specific instance of the trigger. The id is returned in of the body in the [Trigger event webhook](#trigger-event-webhook) |
...
This endpoint can be used to update a specific user's connection. The request must be [authenticated](#authentication). Note, the PUT will replace the current stored configurations for the user. If a user has configured a trigger, sending a PUT request without the same trigger configuration will result in removing the user's configuration for the trigger. Likewise, if the user has never configured a query and it is absent from the `user_connection` from the [Show a connection endpoint](#show-a-connection), this endpoint can be used to store new configurations for the user.
...
```http
PUT /v2/connections/C8p3q9T6/user_connection?user_id=123 HTTP/1.1
Host: connect.ifttt.com
IFTTT-Service-Key: 6e7c8978c07a3b5918a237b9b5b1bb70
Content-Type: application/json
...
_Endpoint has been deprecated. Please use the new [Update a Connection](#update-a-connection) endpoint_

**Authentication:** Service-authenticated with user\_id

**URL**: `POST https://connect.ifttt.com/v2/connections/{{connection_id}}`

This endpoint can be used to update certain details about a specific connection. The request must be [authenticated](#authentication). Currently, you can only update [user trigger fields](#user-trigger-fields), [user query fields](#user-query-fields), and [user action fields](#user-action-fields). An attempt to update anything else will result in a `422 Unprocessable Entity`.
...
```http
POST /v2/connections/C8p3q9T6 HTTP/1.1
Host: connect.ifttt.com
IFTTT-Service-Key: 6e7c8978c07a3b5918a237b9b5b1bb70
Content-Type: application/json
...
- **type** (string): The type of field. Must be `triggers`, `queries`, `actions`, or `features`.
- **type_id** (string): The ID for the specific trigger, query, action, or feature.
...
```http
GET /v2/connections/C8p3q9T6/actions/acme_cloud_storage.archive/field_options?user_id=123 Host: connect.ifttt.com IFTTT-Service-Key: vFRqPGZBmZjB8JPp3mBFqOdt
```
...
(string) A human-readable label for the option.

`value`

(string) An option value to be used in an [action run](#run-an-action) or to [update a connection](#update-a-connection).

`group`

(optional string) A group name which can be used to organize related options when displaying to a user.
...
```http
POST /v2/connections/C8p3q9T6/user_connection/refresh?user_id=123 HTTP/1.1
Host: connect.ifttt.com
...
Connections are enabled by your users on the IFTTT web app or through your mobile app via IFTTT SDKs. When this happens IFTTT will send a request to your API's [Webhook](#webhook-endpoint) endpoint.
...
When a user updates a connection, IFTTT will send a request to your API's [Webhook](#webhook-endpoint) endpoint.
...
When a user disabled a connection, IFTTT will send a request to your API's [Webhook](#webhook-endpoint) endpoint.
...
When a connection is enabled by your users on the IFTTT web app or through your mobile app via IFTTT SDKs, IFTTT will collect and store user configured trigger field values. You can see the currently stored trigger field values for a user via the [show a connection](#show-a-connection) endpoint.

###### Update stored trigger fields

Stored user trigger field values can be updated using the [update a connection](#update-a-connection) endpoint. To update stored fields you must use the [field options](#field-options) endpoint to get valid values for the field.

##### Trigger event webhook

When a trigger detects a new event, IFTTT will send a request to your API's [Webhook](#webhook-endpoint) endpoint.
...
This endpoint can be used to simulate a new event. A request to this endpoint will result in a test [Trigger event webhook](#trigger-event-webhook) request to your API's [Webhook](#webhook-endpoint) endpoint.
...
```http
POST /v2/connections/C8p3q9T6/triggers/acme_cloud_storage.new_file_added/test HTTP/1.1
Host: connect.ifttt.com
IFTTT-Service-Key: vFRqPGZBmZjB8JPp3mBFqOdt
Content-Type: application/json
...
{
  "user_id": 123,
  "user_feature_id": "90276cb1-e8ff-4718-a34f-c73cf75c49cf"
}
```
...
(optional string) If provided, the connection will be run with this configuration. If not provided, the connection's first `user_feature_id` will be used.
...
When a connection is enabled by your users on the IFTTT web app or through your mobile app via IFTTT SDKs, IFTTT will collect and store user configured query field values required to perform a query. You can see the currently stored query field values for a user via the [show a connection](#show-a-connection) endpoint.

###### Update stored query fields

Stored user query field values can be updated using the [update a connection](#update-a-connection) endpoint. To update stored fields you must use the [field options](#field-options) endpoint to get valid values for the field.

###### Overriding stored query fields

Depending on the experience you're looking to create, you may want to override the stored query field values. To do this, include a valid field `label` and `value` in the body of the [perform a query](#perform-a-query) request. View an example of when to override stored fields.
...
This endpoint performs the specified query using the [user query fields](#user-query-fields) stored on IFTTT.
...
```http
POST /v2/connections/C8p3q9T6/queries/acme_cloud_storage.list_files/perform HTTP/1.1
Host: connect.ifttt.com
IFTTT-Service-Key: vFRqPGZBmZjB8JPp3mBFqOdt
Content-Type: application/json
  ...
  "data": [
    {
      ...
      "tags": {
        "type": "query",
        "query_id": "acme_cloud_storage.get_file_metadata",
        "fields": {
...
(object) A map of query fields. This is effectively query parameters.
...
(optional number) A limit for the number of items in the response.

`cursor`

(optional string) A cursor that addresses a specific page of a result set.
...
(string) The type of the response object. Can be either a `list` for a result set, or a `query` for an ingredient that is a query.

`data`

(optional array) The query result as a list of items. Only present if `type` is `list`.

`cursor`

(optional string) A cursor that can be used to request the next page of this result set. Only present if there is a next page.

`fields`

(object) A map of query fields to be used when making a next page or drill-down query request.
...
When a connection is enabled by your users on the IFTTT web app or through your mobile app via IFTTT SDKs, IFTTT will collect and store user configured action field values required to run an action. You can see the currently stored action field values for a user via the [show a connection](#show-a-connection) endpoint.

###### Update stored action fields

Stored user action field values can be updated using the [update a connection](#update-a-connection) endpoint. To update stored fields you must use the [field options](#field-options) endpoint to get valid values for the field.

###### Overriding stored action fields

Depending on the experience you're looking to create, you may want to override the stored action field values. To do this, include a valid field `label` and `value` in the body of the [run an action](#run-an-action) request. View an example of when to override stored fields.
...
This endpoint runs the specified action using the [user action fields](#user-action-fields) stored on IFTTT. If successful, you'll receive an empty 204 No Content response.
...
```http
POST /v2/connections/C8p3q9T6/actions/acme_cloud_storage.upload_from_url/run HTTP/1.1
Host: connect.ifttt.com
IFTTT-Service-Key: vFRqPGZBmZjB8JPp3mBFqOdt
Content-Type: application/json
...
{
  "user_id": 123,
  "user_feature_id": "4da42aa6-3858-4722-9ded-6a7ecdefd91f"
}
```
...
```http
POST /v2/connections/C8p3q9T6/queries/weather.current_weather/perform HTTP/1.1
Host: connect.ifttt.com
IFTTT-Service-Key: vFRqPGZBmZjB8JPp3mBFqOdt
Content-Type: application/json
...
```http
POST /v2/connections/C8p3q9T6/queries/acme.population/perform HTTP/1.1
Host: connect.ifttt.com
IFTTT-Service-Key: vFRqPGZBmZjB8JPp3mBFqOdt
Content-Type: application/json
...
```http
GET /v2/connections/C8p3q9T6/actions/acme.send_message/field_options HTTP/1.1
Host: connect.ifttt.com
IFTTT-Service-Key: vFRqPGZBmZjB8JPp3mBFqOdt
```
...
```json
{
  ...
  "options": {
    "channel": [
      {
        "label": "Cats",
        "value": "oFro6MQAAA1UEwzVVNNVnJ1UtZ2hvc3RidJz",
        "group": null
      },
      {
        "label": "Dogs",
        "value": "EMHK+6aCAPAAAAgkI0WXJ1bpbtcHJpdmF0ZQ",
        "group": null
      }
    ]
  }
...
Then use a value returned in a subsequent query or action run.

**HTTP Request**

```http
POST /v2/connections/C8p3q9T6/actions/acme.send_message/run HTTP/1.1
Host: connect.ifttt.com
IFTTT-Service-Key: vFRqPGZBmZjB8JPp3mBFqOdt
Content-Type: application/json

{
  ...
  "user_feature_id": "4da42aa6-3858-4722-9ded-6a7ecdefd91f",
  "fields": {
    "channel": "oFro6MQAAA1UEwzVVNNVnJ1UtZ2hvc3RidJz",
    "message": "Woof!"
  }
}
```

You will receive an error response if you use a value not from a field options response.
...
```json
{
  ...
  "details": [
    {
      "parameter": "/fields/channel",
      "message": "Not a valid option for this field. Use the field options request to get valid options."
    }
  ]
}
```
...
###### Example: Using a multiple checkbox select (field\_type: CHECKBOX\_MULTI) field when running an action

If you want to override a multiple checkbox field when running an action or query you **must** use a value provided by the `field_options` endpoint. First request all possible field values.
...
```json
{
  ...
  "options": {
    "channel": [
      ...
      {
        "label": "Otter",
        "value": "UQ2Ck3aaHsWBSeszDihhrMxpJPFf1OWL0d9",
        "group": null
      }
    ]
  }
...

{
  ...
  "user_feature_id": "4da42aa6-3858-4722-9ded-6a7ecdefd91f",
  "fields": {
    "channel": ["oFro6MQAAA1UEwzVVNNVnJ1UtZ2hvc3RidJz", "UQ2Ck3aaHsWBSeszDihhrMxpJPFf1OWL0d9"]
  }
}
```
