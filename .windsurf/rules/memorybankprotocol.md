...
7. **Locking Instruction:** ALWAYS use absolute paths for memory-bank files in `/home/kidpixel/SwitchBot/memory-bank/`. Use EXCLUSIVELY the fast-filesystem MCP tools (fast_*). Do NOT attempt to read memory-bank files via regular filesystem tools (read_file).
