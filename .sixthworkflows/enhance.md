...
1. **Analyse Contextuelle & Détection d'Intention**
   ...
   - **Détection de Skill** : Analyser la nature de la tâche :
     ...
     - Si **Architecture** : Identifier le sous-domaine concerné et charger les SKILL workspace pertinents (`.sixthskills/scheduler-ops/SKILL.md` pour SchedulerService, `.sixthskills/postgres-store-maintenance/SKILL.md` pour les stores/DB, `.sixthskills/switchbot-api-dev/SKILL.md` pour l'API/HMAC, `.sixthskills/performance-audit-runbook/SKILL.md` pour les architectures performance). Mentionner explicitement les SKILL consultés dans le prompt.
     - Si **Feature** : Charger `.sixthskills/add-feature/SKILL.md` puis compléter avec les SKILL métier associés (ex. `history-dashboard-updater`, `quota-alerting`, `loader-patterns`, `ifttt-cascade`, `automation-diagnostics`) selon la fonctionnalité visée. Lister ces SKILL dans le prompt final.

2. **Recherche Active de Documentation**
   - Identifier les règles spécifiques au projet via `code_search` dans `docs/` et `.sixthrules/01-coding-standards.md`.
