# SwitchBot Dashboard v2 – Sixth Coding Standards
...
## Sixth Skills Usage
- **Skills locaux en premier** (`.sixthskills/`) :
...
## Skill System Integration (Hybrid Rules + Skills)

When task matches these patterns, automatically load corresponding skill:

- **Bug investigation** (bug, crash, erreur, performance, unexpected behavior) → load `debugging-strategies`
- **Feature development** (new feature, add functionality, implement) → load `add-feature`  
- **Performance issues** (slow, optimization, profiling) → load `performance-audit-runbook`
- **Automation diagnostics** (automation service, scheduler, windows) → load `automation-diagnostics`
- **API development** (switchbot API, HMAC, quota) → load `switchbot-api-dev`
- **Database operations** (postgres, migrations, store) → load `postgres-store-maintenance`
- **History/Charts** (history service, Chart.js, dashboard) → load `history-dashboard-updater`
- **Quota management** (API quota, tracking, alerts) → load `quota-alerting`
- **UI Loaders** (data-loader, ARIA, UX) → load `loader-patterns`
- **IFTTT integration** (webhooks, scenes, cascade) → load `ifttt-cascade`
- **Scheduler operations** (start/stop/reschedule, healthchecks) → load `scheduler-ops`
...
- Chaque fois que vous créez ou modifiez de la documentation (README, docs/, guides Markdown), vous **devez** appliquer la méthodologie définie dans `.sixthskills/documentation/SKILL.md` (TL;DR d'abord, ouverture orientée problème, blocs ❌/✅, tableaux de compromis, Golden Rule). Considérez ce fichier skill comme la checklist de référence avant toute rédaction.
