...
- **Reading**: Utilisez l'outil `mcp0_fast_read_file` avec le paramètre `path="/absolute/path/to/file"`.
  - Priorité 1 : `activeContext.md` (toujours lire en premier).
  - Priorité 2 : `systemPatterns.md` (uniquement pour les choix d'architecture).
- **Updating**: Utilisez l'outil `mcp0_fast_edit_block` avec `path="/absolute/path/to/file"` pour synchroniser l'état du projet en fin de tâche.
- **Writing**: Utilisez l'outil `mcp0_fast_write_file` avec `path="/absolute/path/to/file"` pour l'archivage ou l'initialisation (voir restrictions dans memorybankprotocol.md).
- **Listing projects**: Utilisez l'outil `mcp0_fast_get_directory_tree` avec `path="/absolute/path/to/memory-bank"` pour diagnostiquer la connexion.
- **Listing files**: Utilisez l'outil `mcp0_fast_list_directory` avec `path="/absolute/path/to/memory-bank"` pour lister les fichiers disponibles.
...
### 2.7 Advanced Tools (New Architecture)
- **Task Management**: `task-master-ai` for project planning and backlog management
- **Sequential Thinking**: `sequentialthinking_tools` for logical decomposition and validation
- **Fast Filesystem**: `fast_edit_block`, `fast_read_multiple_files` for token-efficient operations
- **JSON Query**: `json_query_jsonpath`, `json_query_search_keys` for precise JSON manipulation
- **Render Service Management**: `render-switchbot-mcp` for Render.com operations (deployments, cron jobs, monitoring, environment variables)
- **PostgreSQL Operations**: `switchbot-postgres` for advanced database operations (health analysis, indexes, secure queries, schema exploration)

### 2.8 Complexity Locks and Constraints
- **Verrou de Complexité**: Si la demande implique une fonctionnalité majeure ou un projet entier, l'IA a l'obligation de demander l'exécution de `enhance_complex`.
- **Gestion JSON**: Interdiction de lire intégralement (`read_file`) les fichiers JSON volumineux (manifest, i18n). Utilise `json-query` pour extraire uniquement les nœuds pertinents avant modification.
- **Token Efficiency**: Pour les fichiers >1000 lignes, utilisation obligatoire de `fast_edit_block` plutôt que `edit_file` standard.
- **Sequential Validation**: Pour toute tâche complexe, validation logique obligatoire via `sequentialthinking_tools` avant implémentation.
