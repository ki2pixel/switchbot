...
- Chaque fois que vous créez ou modifiez de la documentation (README, docs/, guides Markdown), vous **devez** appliquer la méthodologie définie dans `.continue/rules/documentation.md` (TL;DR d'abord, ouverture orientée problème, blocs ❌/✅, tableaux de compromis, Golden Rule). Considérez ce fichier skill comme la checklist de référence avant toute rédaction.
